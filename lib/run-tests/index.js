...
const chunk = CHUNKS_FILE ? new Set(require(CHUNKS_FILE)[CHUNK]) : undefined;
...
async function main() {
  ...
  const agents = new AgentPool(NODE, THREADS);
    ...
    agents.withAgent(async (agent) => {
      ...
      if (actual.result === expected) {
        ...
        tap.fail(
          ...
          actual.error ? new RethrownError(actual.error) : new Error("[no error]")
        );
      }
    });
  }
