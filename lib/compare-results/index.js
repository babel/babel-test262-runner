...
function parseTestResults(filePath) {
  return new Promise(resolve =>
    _(fs.createReadStream(filePath).pipe(parser.stream()))
...
async function main() {
  ...
  if (!(mainArtifactFileArg && prArtifactFileArg)) {
    throw new Error(
      `main branch and PR artifact file paths required. Usage: node lib/compare-results main.tap test262.tap`
    );
  }
  ...
  const [mainTests, prTests] = await Promise.all([
    parseTestResults(mainArtifactFilePath),
    parseTestResults(prArtifactFilePath),
  ]);

  if (mainTests.length !== prTests.length) {
    diag(
      `main branch and PR artifacts are out of sync. Consider rebasing the PR.`
    );
  }
    ...
    switch (prTest.type) {
      case "assertion":
        if (!(fileName in mainTestsMap)) {
          diag(
            `Ignoring test '${fileName}' as it was not found in main artifact!`
          );
          continue;
        }
